...
const JOBS: Job[] = [
  ...
  { id: '6', title: 'Secondary School Teacher (Mathematics)', company: 'Arusha Municipal Council', location: 'Arusha', country: 'Tanzania', flag: '🇹🇿', sector: 'government', type: 'full-time', salary: 'TZS 600K–900K/month', deadline: 'Jun 25, 2026', description: 'Teach O-level and A-level Mathematics in government secondary schools.', requirements: ['B.Ed. Mathematics or BSc with PGDE', 'Registered with NECTA', 'Swahili and English'], tags: ['Education', 'Teaching', 'Government'] },
  { id: '7', title: 'Project Coordinator', company: 'World Food Programme (WFP)', location: 'Dodoma', country: 'Tanzania', flag: '🇹🇿', sector: 'ngo', type: 'contract', salary: 'USD 2,000–3,000/month', deadline: 'Jun 18, 2026', description: 'Coordinate food security and nutrition projects in central Tanzania.', requirements: ['Degree in Agriculture, Nutrition or Development', '3+ years project management', 'Swahili required', 'Driving licence'], tags: ['NGO', 'Food Security', 'Coordination'] },
  { id: '8', title: 'Human Resources Manager', company: 'Tanzania Breweries Limited', location: 'Dar es Salaam', country: 'Tanzania', flag: '🇹🇿', sector: 'private', type: 'full-time', salary: 'TZS 3M–5M/month', deadline: 'Jul 1, 2026', description: 'Lead HR strategy, recruitment, and employee relations at TBL.', requirements: ['Degree in HRM or Business', 'CIPD or equivalent', '5+ years HR experience', 'Manufacturing sector preferred'], tags: ['HR', 'Management', 'FMCG'] },
  { id: '9', title: 'Monitoring & Evaluation Officer', company: 'PATH Tanzania', location: 'Dar es Salaam', country: 'Tanzania', flag: '🇹🇿', sector: 'ngo', type: 'full-time', salary: 'USD 1,800–2,800/month', deadline: 'Jun 22, 2026', description: 'Design and implement M&E systems for health programmes across Tanzania.', requirements: ['Masters in Public Health, Statistics or related', '3+ years M&E experience', 'SPSS or Stata', 'ODK or KoboToolbox'], tags: ['NGO', 'M&E', 'Health'] },
  { id: '10', title: 'Civil Engineer', company: 'Tanzania Roads Authority (TANROADS)', location: 'Dodoma', country: 'Tanzania', flag: '🇹🇿', sector: 'government', type: 'full-time', salary: 'TZS 1.5M–2.5M/month', deadline: 'Jul 15, 2026', description: 'Design and supervise road and infrastructure projects across Tanzania.', requirements: ['BSc Civil Engineering', 'ERB registered', '3+ years experience', 'AutoCAD proficiency'], tags: ['Engineering', 'Government', 'Infrastructure'] },
  { id: '11', title: 'Sales Representative', company: 'Azam Media', location: 'Dar es Salaam', country: 'Tanzania', flag: '🇹🇿', sector: 'private', type: 'full-time', salary: 'TZS 800K–1.5M + commission', deadline: 'Jul 8, 2026', description: 'Drive subscription sales and partnerships for Azam TV services.', requirements: ['Diploma or Degree in Sales/Marketing', '1+ year sales experience', 'Good communication skills', 'Own motorcycle preferred'], tags: ['Sales', 'Media', 'Marketing'] },
  { id: '12', title: 'Pharmacist', company: 'Muhimbili National Hospital', location: 'Dar es Salaam', country: 'Tanzania', flag: '🇹🇿', sector: 'government', type: 'full-time', salary: 'TZS 1.2M–2M/month', deadline: 'Jun 28, 2026', description: 'Dispense medications and advise clinical staff at Tanzania\'s largest referral hospital.', requirements: ['B.Pharm', 'Registered with Pharmacy Council of Tanzania', '1+ year experience'], tags: ['Health', 'Pharmacy', 'Government'] },
  { id: '13', title: 'Digital Marketing Specialist', company: 'Selcom', location: 'Dar es Salaam', country: 'Tanzania', flag: '🇹🇿', sector: 'tech', type: 'full-time', salary: 'TZS 1.5M–2.5M/month', deadline: 'Jul 3, 2026', description: 'Drive digital marketing campaigns for Tanzania\'s leading payment platform.', requirements: ['Degree in Marketing or Communications', 'Google Ads, Meta Ads experience', 'Content creation skills', '2+ years digital marketing'], tags: ['Marketing', 'Digital', 'Fintech'] },
...
export default function JobsScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return jobs;
  }, [search, sectorFilter, countryFilter, activeTab, state.savedJobs, state.cv.targetSector]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={{ paddingHorizontal: 16, gap: 8 }}>
        <TouchableOpacity
          style={[styles.filterChip, !countryFilter && { backgroundColor: colors.ink, borderColor: colors.ink }]}
        ...
        {COUNTRIES.map(c => (
          ...
          >
            <Text style={[styles.filterChipText, countryFilter === c.id && { color: '#fff' }]}>{c.flag} {c.label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={{ paddingHorizontal: 16, gap: 8 }}>
        <TouchableOpacity
          style={[styles.filterChip, !sectorFilter && { backgroundColor: 'rgba(26,20,16,0.08)', borderColor: 'transparent' }]}
          onPress={() => setSectorFilter(null)}
        >
          <Text style={[styles.filterChipText, !sectorFilter && { color: colors.foreground2 }]}>{t('All Sectors', 'Sekta Zote')}</Text>
        </TouchableOpacity>
        {SECTORS.map(s => (
          <TouchableOpacity
            key={s.id}
          ...
          >
            <Text style={[styles.filterChipText, sectorFilter === s.id && { color: colors.foreground2 }]}>{s.label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      {selectedJob && (
        <JobDetailModal job={selectedJob} visible={!!selectedJob} onClose={() => setSelectedJob(null)} lang={lang} />
      )}
...
function JobDetailModal({ job, visible, onClose, lang }: { job: Job; visible: boolean; onClose: () => void; lang: string }) {
  ...
  const generateLetter = async (type: 'cover' | 'app') => {
    ...
    const prompt = type === 'cover'
      ? `Write a professional cover letter in English for a job applicant in East Africa (${job.country}).
Job: ${job.title} at ${job.company}, ${job.location}.
Applicant: ${cv.firstName} ${cv.lastName}, Skills: ${cv.skills.map(s => s.name).join(', ')}, Experience: ${cv.experienceLevel}.
Make it compelling, 3 paragraphs, East Africa context. Do not add placeholders.`
      : `Write a formal application letter in both English and Kiswahili (provide both versions) for:
        ...
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          ...
          <View style={detailStyles.actionGrid}>
            <TouchableOpacity
              ...
              onPress={() => generateLetter('cover')} disabled={loading}
            >
            ...
            <TouchableOpacity
              style={[detailStyles.actionBtn, { backgroundColor: colors.sand, flex: 1 }]}
              onPress={() => generateLetter('app')} disabled={loading}
            >
          ...
          <TouchableOpacity
            ...
            onPress={handleApply} disabled={isApplied} activeOpacity={0.85}
          >
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <View style={[detailStyles.container, { backgroundColor: colors.background }]}>
        <View style={[detailStyles.header, { borderBottomColor: colors.border }]}>
          <TouchableOpacity onPress={onClose}><Ionicons name="chevron-down" size={26} color={colors.foreground} /></TouchableOpacity>
          <Text style={[detailStyles.title, { fontSize: 16, color: colors.foreground }]}>🌍 {t('East Africa Salary Guide', 'Mwongozo wa Mishahara')}</Text>
          <View style={{ width: 26 }} />
        </View>
        <ScrollView contentContainerStyle={{ padding: 16 }}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }} contentContainerStyle={{ gap: 8 }}>
            {(Object.entries(countryLabels) as [typeof selectedCountry, string][]).map(([code, label]) => (
              <TouchableOpacity
                key={code}
          ...
          <Text style={[detailStyles.body, { color: colors.muted, marginBottom: 16 }]}>
            {t('Market salary ranges for', 'Viwango vya mishahara')} {countryLabels[selectedCountry]} (2026)
          </Text>
          {SALARY_DATA.map(s => (
            <View key={s.role} style={[detailStyles.salaryBadge, { backgroundColor: colors.card, borderRadius: 14, marginBottom: 10, flexDirection: 'row', justifyContent: 'space-between' }]}>
              ...
              <Text style={[detailStyles.salary, { color: colors.success, textAlign: 'right', flexShrink: 1 }]}>{s[selectedCountry]}</Text>
            </View>
          ))}
        </ScrollView>
      </View>
