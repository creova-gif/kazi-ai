...
- Target sectors: ${cv.targetSector.join(', ') || 'Unknown'}`;
  ...
  const send = async (text: string) => {
    ...
    try {
      const text = await callAI({ max_tokens: 500, system: SYSTEM, messages: apiMessages });
      const reply = text.trim() || (lang === 'sw' ? 'Samahani, hitilafu imetokea.' : 'Sorry, an error occurred.');
